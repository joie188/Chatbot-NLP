...
class TorchGeneratorAgent(TorchAgent, ABC):
    ...
    def _generate(
        ...
        for _ts in range(max_ts):
            ...
            # src_mask = (self._encoder_input(batch)[0] != 0).long().unsqueeze(-2)
            # encoder_states = torch.stack(list(encoder_states), dim=0)
            score, incr_state = model.decoder(decoder_input, encoder_states, incr_state)
            # score, incr_state = model.decoder(decoder_input, torch.vstack(encoder_states), encoder_states[-1], src_mask)
